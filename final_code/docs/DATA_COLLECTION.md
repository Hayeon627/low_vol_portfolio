...
> ### ⚠️ 본 문서의 수치는 논문 데이터 빈티지 기준
> 아래 종목 수(유니버스 833 / daily_returns 824 / monthly_panel 617)는 **논문 실행 시점**의 값이다.
> `build_monthly_membership()` 이 Wikipedia **현재** 페이지에서 과거 S&P 500 편입·편출 이력을 재구성하므로,
> 노트북을 다시 실행하면 과거 유니버스가 달라진다 (2026-08-07 재수집 기준 837 / 836 / **609**).
> **01 을 재실행하면 04_BL_Walkforward 이후 논문 성과 수치를 재현할 수 없다.**
> HMM(05a)은 FF·매크로만 사용하므로 유니버스 변동의 영향을 받지 않는다.
